...
var token = '695795109.ca477ed.81debc29770746ef9493c02437cf6007';
var userId = '695795109';

async function getData() {
  try {
    var response = await fetch('https://api.instagram.com/v1/users/self/media/recent/?access_token=' + token);
    if (response.ok) {
...
$(document).ready(function () {
  ...
  if (instafeed.length > 0) {
    var instaMakeSquare = function instaMakeSquare() {
    ...
    getData().then(function (result) {
      ...
      for (var i = 0; i < 6; i++) {
        $('#instafeed .grid-x').append('<div class="cell small-6 large-4">' + '<a href="' + result[i].link + '" target="_blank">' + '<div class="instafeed-image-wrapper">' + '<img src="' + result[i].images.low_resolution.url + '" />' + '</div>' + '</a>' + '</div>');
      }
    }).then(function () {
      instaMakeSquare();
    });
